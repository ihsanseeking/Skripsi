								...
								<div id="identifikasi_table">
									...
									<?php
										...
										if ($n_ident_tbl > 1){
											...
											echo "Ada Relasi";
											for ($i_ident_tbl = 1; $i_ident_tbl <= $n_ident_tbl; $i_ident_tbl++){
												echo "<br>FROM `$ident_table[$i_ident_tbl]`<br>";
												$rel_n_pilihan = count($rel_pilihan[$ident_table[$i_ident_tbl]]);
												for ($rel_i = 1; $rel_i <= $rel_n_pilihan; $rel_i++){ //Looping Relasi
													echo "<br> Relasi $ident_table[$i_ident_tbl] = ".$rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["nama"];
													//Cek Relasi ke di sendiri
													if ($rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["foreign_table"] == $rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["references_table"]){
														//echo "<br> Ada Relasi ke diri sendiri = ".$rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["nama"];
													}
													//Cek Relasi sebagai Foreign
													if ($rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["foreign_table"] == $ident_table[$i_ident_tbl]){
														//echo "<br>Sebagai foreign untuk = ".$rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["references_table"];
														...
														//for ($i_ident_tbl = 1; $i_ident_tbl <= $n_ident_tbl; $i_ident_tbl++){ //Looping ident_table dari > 1
															//if ($rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["references_table"] == $ident_table[$i_ident_tbl]){
																//echo "<br> Ada Relasi langsung sebagain Foreign = ".$rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["nama"];
															//}
														//}
													}
													//Cek Relasi Sebagai References
													if ($rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["references_table"] == $ident_table[$i_ident_tbl]){
														//echo "<br>sebagai references dari = ".$rel_pilihan[$ident_table[1]][$rel_i]["foreign_table"];
														// cek adakah ident_table > 1 di foreign dengan references $ident_table[1]?
														//for ($i_ident_tbl = 1; $i_ident_tbl <= $n_ident_tbl; $i_ident_tbl++){ //Looping ident_table dari > 1
															//if ($rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["foreign_table"] == $ident_table[$i_ident_tbl]){
																//echo "<br> Ada Relasi langsung sebagain references = ".$rel_pilihan[$ident_table[$i_ident_tbl]][$rel_i]["nama"];
															//}
														//}
													}
												}
